...
func TestRolePropsOverrideDefaultLandmarkSemantics(t *testing.T) {
	tree := SourceTree{SurfaceID: "s", Root: SourceNode{ID: "root", Kind: "application", Children: []SourceNode{
		...
		{ID: "heading", Kind: "text", Props: map[string]any{"text": "Details", "role": "heading", "ariaLevel": 2, "tabIndex": 1}},
	...
	if projected.Root.Children[1].Role != RoleHeading || projected.Root.Children[1].States["level"] != "2" || projected.Root.Children[1].States["focusOrder"] != "1" {
		t.Fatalf("role, level, and focus order props should project heading semantics: %#v", projected.Root.Children[1])
	}
