...
Native terminal modal fallback is document-first: when a modal frame includes a `document`, the host renders the structured document before considering legacy `body` text. The terminal projection includes layout/chrome primitives (`surface`, `window`, `viewport`, `split`, `scroll`, `section`, `box`, `disclosure`, `separator`, `spacer`, `icon`, `badge`, `breadcrumb`, `contextMenu`), cards, progress-like controls, tables, lists, trees, timelines, logs, tabs, command palettes, keybinding hints, pagination/help/prompt controls, key/value details, media/embed placeholders (`image`, `video`, `chart`, `terminal`, `canvas`, `extensionOutlet`), status fallbacks (`toast`, `empty`, `spinner`, `loading`, `errorBoundary`), and form controls (`textInput`, `passwordInput`, `searchInput`, `numberInput`, `dateInput`, `fileInput`, `textArea`, `select`, `radioGroup`, `checkbox`, `toggle`, and `slider`) as sanitized text rows. Password values are masked in the projection.
