...
Trace output is deterministic and metadata-first. When Black Box is installed, the viewer reads its metadata-only JSONL records from `extension-data\black-box` and keeps prompt, response, source, token, and tool body fields redacted.
