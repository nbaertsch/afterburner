...
const capture = 'C:\\Users\\noahbaertsch\\.afterburner\\copilot-home\\session-state\\b518acc2-e4e8-4dac-ab53-f9b09666fce6\\files\\installed-openai-server-repro';
...
const child = pty.spawn(afterburn, ['--disable-extension', 'black-box', '--disable-extension', 'byo-models', '--disable-extension', 'steward-burn', '--name', `installed-openai-server-repro-${process.pid}-${Date.now()}`, '--no-remote'], { name: 'xterm-256color', cols: 140, rows: 40, cwd: process.cwd(), env });
const inputs = [];
...
function finish(code, msg) {
  ...
  const result = { code, msg, sent, nativeModal: /OpenAI Server[\s\S]*Native Afterburner modal overlay/i.test(text), canvasOpened: /Canvas opened:\s*OpenAI Server/i.test(text), unavailable: /native menu unavailable|interactive menu could not open/i.test(text), activated: /activated Afterburner extension 'openai-server'/i.test(text), inputs, tail: text.slice(-5000) };
...
child.onData(data => {
  ...
  if (!scheduled && /activated Afterburner extension 'openai-server'/i.test(text) && /\/ commands|tab next tab|\? help/i.test(recent)) {
    scheduled = true;
    setTimeout(() => {
      sent = true;
      write('\x15', 'clear prompt');
      write('\x1b[200~/openai-server\x1b[201~\r', 'bracket-paste submit command');
    }, 5000);
  }
  if (sent && /OpenAI Server[\s\S]*Native Afterburner modal overlay/i.test(text)) finish(0, 'native modal opened in installed afterburn');
  if (sent && /Canvas opened:\s*OpenAI Server/i.test(text)) finish(2, 'only generic canvas opened');
